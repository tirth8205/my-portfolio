...
export default function ExperienceCard({ experience }: Props) { // Correctly destructures 'experience'
  ...
  return (
    <article className="flex flex-col rounded-3xl items-center space-y-2 xs:space-y-3 md:space-y-4 flex-shrink-0
                       ...
                       snap-center bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700
                       p-4 xs:p-5 md:p-8 shadow-lg hover:shadow-xl transition-all duration-200 overflow-hidden">
        ...
        <div className="md:flex md:justify-between items-start">
          <div className="md:flex-1">
            <div className="md:flex md:items-center md:justify-between">
              <h4 className="text-lg md:text-2xl font-light text-darkBlack dark:text-white">
            ...
            <p className="font-bold text-md md:text-xl mt-1 text-accent">
              {experience?.company || "Company Name"}
            </p>

            <p className="md:hidden uppercase py-1 text-gray-500 dark:text-gray-400 text-xs">
              {formatDate(experience?.dateStarted)} -{" "}
              {experience.isCurrentlyWorkingHere
                ? "Present"
                : formatDate(experience?.dateEnded)}
            ...
            <div className="flex flex-wrap gap-2 my-2 md:my-3">
              {experience?.technologies?.map((technology) =>
                isValidImage(technology?.image) ? (
                  <div key={technology._id} className="relative h-8 w-8 sm:h-10 sm:w-10"> 
                    <SanityImage
      ...
      {pointsToRender.length > 0 && (
        <div className="w-full px-1 sm:px-2 md:px-4 max-h-[150px] xs:max-h-[170px] md:max-h-[180px] overflow-hidden mt-2 md:mt-3">
          <ul className="list-disc text-darkBlack dark:text-gray-300 space-y-1.5 text-left text-sm md:text-base
                         pl-5 h-full overflow-y-auto scrollbar-thin scrollbar-track-gray-200 dark:scrollbar-track-gray-700 scrollbar-thumb-accent">
