  ...
  return (
    <motion.div
      ...
      className="h-screen relative flex flex-col items-center text-left max-w-full mx-auto z-0 overflow-hidden 
                 pt-32 sm:pt-36 md:pt-40 pb-8 sm:pb-10 md:pb-12 px-4"
    >
      ...
      <div className="relative w-full flex-1 flex flex-col items-center overflow-hidden mt-2 sm:mt-4">
        ...
        <div
          ...
          className="w-full h-full flex overflow-x-scroll overflow-y-hidden snap-x snap-mandatory z-20 scrollbar-thin scrollbar-track-gray-400/20 scrollbar-thumb-darkGreen/80"
        >
          {projects?.map((project, i) => (
            <div
              ...
              className="w-full h-full flex-shrink-0 snap-center flex flex-col items-center 
                         p-4 pt-6 sm:p-5 sm:pt-6 md:p-6 md:pt-8 box-border" // Changed w-screen to w-full, added box-border
            >
              <div className="relative h-28 sm:h-32 md:h-40 lg:h-44 w-full max-w-xs sm:max-w-sm md:max-w-[460px] lg:max-w-[500px] mx-auto flex-shrink-0 mb-2 md:mb-3">
                {isValidImage(project?.image) ? (
                  <SanityImage asset={project.image} alt={project?.title || "Project image"} layout="fill" objectFit="contain"/>
                ) : (
                  ...
                  {project?.technologies?.map((technology) => (
                    isValidImage(technology?.image) ? (
                      <div key={technology._id} className="h-5 w-5 sm:h-6 sm:w-6 relative">
                        <SanityImage asset={technology.image} alt={technology.title || "Technology"} layout="fill" objectFit="cover" className="rounded-full"/>
                      </div>
                    ) : null
                  ))}
                </div>
        ...
        <div className="absolute bottom-1 sm:bottom-2 flex justify-center space-x-2 md:hidden z-20">
          {projects?.map((_, index) => (
            <button key={index} onClick={() => scrollTo(index)} className={`w-2 h-2 sm:w-2.5 sm:h-2.5 rounded-full transition-colors ${index === currentIndex ? 'bg-darkGreen' : 'bg-gray-400 hover:bg-gray-500'}`} aria-label={`Go to project ${index + 1}`}/>
          ))}
        </div>
      </div>
